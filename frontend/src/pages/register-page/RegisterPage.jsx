 ...
 * При успешной регистрации добавляет параметр в URL для уведомления HomePage.
 *
 * @module RegisterPage
 */
...
function RegisterPage({ onLogin }) {
   ...
   * При успехе, вызывает onLogin, перенаправляет и добавляет параметр в URL.
   *
   * @async
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (res_body.success && res_body.data.token) {
        ...
        // --- Изменения здесь ---
        // Добавляем параметр в URL, чтобы HomePage увидел, что нужно показать модальное окно
        const url = "/?confirmEmail=true";
        navigate(url); 
        // --- Конец изменений ---
